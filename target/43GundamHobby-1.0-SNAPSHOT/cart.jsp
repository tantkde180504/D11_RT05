...
<div class="cart-container">
    ...
    <div style="text-align:right; margin-top:20px;">
        <button class="checkout-btn" type="button" disabled>Thanh toán</button>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Lắng nghe sự kiện click các nút tăng/giảm/xóa
    document.querySelector('.cart-table').addEventListener('click', function(e) {
        ...
        if (row) {
            ...
            if (qtyInput) currentQty = parseInt(qtyInput.value) || 1;
        }
        if (action === 'increase') {
            fetch('/api/cart/update', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({productId: productId, quantity: currentQty + 1})
            })
            .then(res => res.json())
